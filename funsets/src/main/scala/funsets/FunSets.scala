...
object FunSets {
   ...
   */
  def singletonSet(elem: Int): Set = (otherele: Int) => elem == otherele
   ...
   */
  def filter(s: Set, p: Int => Boolean): Set = (otherele: Int) => p(otherele) && contains(s, otherele)
   ...
   */
  def map(s: Set, f: Int => Int): Set = (otherele: Int) => exists(s, (elem: Int) => f(elem) == otherele)
