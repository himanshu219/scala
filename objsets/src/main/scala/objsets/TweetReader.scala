...
object TweetReader {
  ...
  object ParseTweets {
    ...
    def getList[T](s: String): List[T] =
      JSON.parseFull(s).get.asInstanceOf[List[T]] //why
    ...
    def getTweetData(user: String, json: String): List[Tweet] = {
      ...
      for (map <- l) yield {
        ...
        new Tweet(user, text.toString, retweets.toString.toDouble.toInt) //why
      }
    }
  }

  def toTweetSet(l: List[Tweet]): TweetSet = {
    l.foldLeft(new Empty: TweetSet)(_.incl(_)) //why
  }
  ...
  val tweetMap: Map[String, List[Tweet]] = //why
